...
export const genPubKey = async (privateKey: string): Promise<string> => {
  const pubKey = await executeSingleCommand('bash', ['-c', `echo ${privateKey} | wg pubkey`]);
  return pubKey;
};

export interface GeneratedClientKeys {
  randomKey: string;
  presharedKey: string;
  pubKey: string;
}

export const genNewClientKeys = async (): Promise<GeneratedClientKeys> => {
