...
import { PeerRecord, PeerStorage } from '../types/config.js';
...
const readPeers = (): PeerStorage => {
  return readJSON({ filePath: path.resolve(PEERS_PATH), parseJSON: true, createIfNotFound: {} }) as PeerStorage;
};
...
export const addNewClient = async (req: Request, res: Response, next: NextFunction): Promise<void> => {
  const requestedIP = req.body?.ip as string | undefined;
  const newName = req.body?.name as string | undefined;
  const iface = req.body?.iface as string;
  try {
...
export const removeClient = async (req: Request, res: Response, next: NextFunction): Promise<void> => {
  const iface = req.body?.iface as string;
...
export const getClientConfig = async (req: Request, res: Response, next: NextFunction): Promise<void> => {
